...
cmd = "git clone https://github.com/ehraz786/save2gdrive"
proc = subprocess.run(cmd, shell=True)
cmd = "apt update && apt install ffmpeg aria2"
proc = subprocess.run(cmd, shell=True)
cmd = "pip3 install -r /content/save2gdrive/requirements.txt"
proc = subprocess.run(cmd, shell=True)
...
with open('/content/save2gdrive/credentials.json', 'w') as file:
    file.write(json.dumps(credentials))

Working = False

if os.path.exists("/content/save2gdrive/my_bot.session"):
    os.remove("/content/save2gdrive/my_bot.session") # Remove previous bot session
    
print("\rStarting Bot....")

!cd /content/save2gdrive/ && python3 -m colab_leecher #type:ignore
